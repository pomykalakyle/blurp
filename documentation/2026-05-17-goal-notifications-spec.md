...
Push notifications on the user's phone that fire when a goal has passed its end date and needs review. Tapping a notification opens the app to a [goal check-in chat](2026-05-17-goal-check-in-chat-spec.md) covering the goal(s) in question, where the assistant has already opened the conversation.

Multiple overdue goals collapse into **one** notification and **one** check-in chat — the user is never spammed with parallel pings for separate goals at the same time.
...
- The system prompt is amended for scoped check-ins to instruct the model to **walk through that specific list** and ask the user the status of each goal in turn (still direct, still no preamble — see check-in spec §4 for the tone).
...
The app is delivered as a Progressive Web App. Once the user has added the app to their phone's home screen, iOS Safari permits the app to register for Web Push notifications via a service worker.

When the backend wants to ping the user, it sends a Web Push message to a stored subscription endpoint. iOS displays the notification on the lock screen. Tapping it opens the PWA at a URL the push payload specifies — deep-linking straight to the relevant goal check-in chat.
...
## 4. Schedule: two fixed user-local time slots

Notifications fire at exactly two times per day, in the user's configured time zone:

- **10:00 AM user-local time**
- **3:00 PM user-local time**
...
2. The goal has an `endDate` and that date is in the past (the user's configured time zone).
...
2. Generates and saves the assistant's opening message server-side (so it's already there when the user taps the notification). The model is given the scope and instructed to ask about each goal directly.
...
**Stop on state change (per goal).** Once a goal's state changes such that it no longer qualifies — marked done, marked slipped, or had its `endDate` extended into the future — it stops appearing in future bundles. This is the implicit "stop on response" rule: if the user answers in the chat and the model proposes a mutation the user accepts, the goal is removed from the qualifying set naturally.

If the user responds in the chat but no mutation is applied (he says "let me think," or the model can't extract a clear answer), the goal stays in the qualifying set and is eligible for the next slot. Not responding at all means the same thing.

**Twice-only per deadline.** Independent of state, the system caps pings at 2 per goal per the current `endDate`. A goal that's been pinged twice and is still unaddressed just sits — no third ping. the user can manually start a check-in to deal with it whenever he wants.

**Re-arming via extension.** If the user (via the assistant) extends a goal's `endDate`, the goal effectively gets a fresh deadline. The ping counter for the previous deadline no longer applies — the goal re-qualifies once the new `endDate` passes, and the 2-ping cycle starts over.
...
This trades a bit of lock-screen privacy (goal titles are briefly visible) for clarity — a count alone gives the user no signal about urgency or what's waiting.

---

## 9. Push subscription management

A small `pushSubscriptions` table in Convex stores the endpoint(s) the user has registered:
...
A first-run flow in the PWA prompts for notification permission, calls `pushManager.subscribe()` with the app's VAPID public key, and saves the resulting subscription to Convex. If the user revokes permission later, the next failed push (the push service returns a `410 Gone`) tells the backend to delete the row.

VAPID keys are generated once during setup. The public key is exposed to the frontend via a Vercel env var. The private key and VAPID subject are stored as Convex env vars.

If the `pushSubscriptions` table is empty at slot fire time, the cron still runs and still creates the check-in chat — the chat exists in the user's check-in list for him to find next time he opens the app. The push is the optional nudge; the chat is the durable artifact.
...
- **Two cron jobs** in `convex/crons.ts`, one each at 10am and 3pm user-local time.
...
- **user-local time is the only time zone.** Both slots are defined in user-local time. If the user travels, notifications still fire on user-local clock — acceptable for v1.
- **No quiet hours feature.** 10am and 3pm are inside reasonable user-local time waking hours by design.
...
- **Push delivered but the user never taps.** The chat exists in his check-in list regardless. The notification is the optional nudge; the chat is the durable artifact.
